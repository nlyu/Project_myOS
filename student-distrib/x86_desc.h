...
/**********************************************************/
/*	        STRUCT & GLOBAL VARIABLE FOR PAGING           */
/**********************************************************/

/* Page-directory entry for size mode 4KB pages table */
...
/* Page entry for size mode 4KB pages */
typedef union pe_4KB {
	uint32_t val;
	struct {
		uint32_t present : 1;
		uint32_t read_write : 1;
...
/* Page directory entry for size mode 4MB pages */
...
/* Page entry directory union type with two different size mode */
typedef union pde {
	pde_4KB KB;
	pde_4MB MB;
} pde;

/* Page directory that contains 1024 page-directory entries */
typedef struct {
	pde pd_entries[PAGE_DIRECTORY_SIZE];
} pdt;

/* page-directories array, 8 directories for 8 process separately. */
pdt page_directories[6] __attribute__((aligned (4096)));

/* Page table entries (declared in x86_desc.S) */
pe_4KB page_table_shell[PAGE_TABLE_SIZE] __attribute__((aligned (4096)));
pe_4KB page_table_video[PAGE_TABLE_SIZE] __attribute__((aligned (4096)));
/**********************************************************/
/*					  END OF PAGING                       */
/**********************************************************/
